...
def train():
    ...
    # 初始化生成器和判别器
    generator = Generator(latent_dim, condition_dim, img_shape).to(device)
    discriminator = Discriminator(img_shape, condition_dim).to(device)
...
def single_train(epoch, generator, discriminator,optimizer_G, optimizer_D, train_loader, adversarial_loss, d_losses, g_losses):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        real_condition = torch.zeros((batch_size, 10)).to(device).scatter_(1, labels.view(-1, 1), 1)
        real_validity = discriminator(real_imgs, real_condition)
        real_loss = adversarial_loss(real_validity, valid)

        # 生成图片
        noise = torch.randn(batch_size, latent_dim).to(device)
        gen_labels = torch.randint(0, 10, (batch_size,)).long().to(device)
        gen_condition = torch.zeros((batch_size, 10)).to(device).scatter_(1, gen_labels.view(-1, 1), 1)
        gen_imgs = generator(noise, gen_condition).to(device)
